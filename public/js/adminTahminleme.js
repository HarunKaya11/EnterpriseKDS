    ...
    if (aracId) {
        fetch(`/api/kiralama-verileri?arac_id=${aracId}`)
            ...
            .then(data => {
                ...
                document.getElementById('2025-gun-sayisi').innerText = data.totalDays[4];
                

                if (data.averagePrediction !== undefined) {
                    document.getElementById('2026-gun').innerText = Math.round(data.averagePrediction+data.averagePrediction/5);
                } else {
        ...
        fetch(`/api/bakim-masrafi?arac_id=${aracId}`)
            ...
            .then(data => {
                ...
                document.getElementById('2025-masraf').innerText = formatTL.format(data.totalCosts[4]);

            

                if (data.averageCost !== undefined) {
                    document.getElementById('2026-masraf').innerText = formatTL.format(data.averageCost+data.averageCost/5);
                } else {
                    console.error("Tahmin değeri undefined.");
                }
            })
        ...
        fetch(`/api/kiralama-geliri?arac_id=${aracId}`)
            ...
            .then(data => {
                ...
                document.getElementById('2025-Gelir').innerText = formatTL.format(data.totalincome[4]);
